#!/usr/bin/env python3
"""Analyze robustness suite outputs and evaluate pass criteria."""
...
PATTERN = re.compile(r"^(R\d+)-([a-z]+)-(baseline|method)-s\d+$")
...
def status_for(test: str, severity: str, baseline: float, method: float, nominal_method: float) -> Tuple[str, str]:
    ...
    if test == "R1" and severity == "med":
        drop = (nominal_method - method) / nominal_method * 100.0
        return ("PASS" if drop <= 10.0 else "FAIL", "method drop <= 10% at med")
    if test == "R2" and severity == "med":
        drop = (nominal_method - method) / nominal_method * 100.0
        return ("PASS" if drop <= 12.0 else "FAIL", "method drop <= 12% at med")
    if test == "R3" and severity in {"mild", "severe"}:
        return ("PASS" if method > baseline else "FAIL", "method > baseline")
    if test == "R4" and severity == "hard":
        return ("PASS" if method >= baseline else "FAIL", "method >= baseline")
...
def main() -> int:
    ...
    lines: List[str] = []
    lines.append("# Robustness Results (Auto-generated)")
    lines.append("")
    ...
    lines.append("")
    lines.append(f"- Required robustness criteria pass: {'YES' if out['all_required_pass'] else 'NO'}")
